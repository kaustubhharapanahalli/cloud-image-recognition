...
queue_url = (
    "https://sqs.us-east-1.amazonaws.com/697190715831/myQueue.fifo"  # queu URL
)
...
def getEC2Data():
    ...
    for instance in ec2.instances.all():
        ...
        ):  # counting the number of EC2s that are running
            running_count += 1
        elif (
            instance.state["Name"] == "stopped"
        ):  # counting the number of EC2s that are stopped
...
while True:
    ...
    )  # fetching information of all the EC2s
    message_count = int(
    ...
    if message_count > prev_message_count:
        (
            ...
            stopped_count,
            stopped_list,
        ) = getEC2Data()  # fetch counts of EC2s

        ec2_needed_count = min(
            20, math.ceil(message_count / 10.0)
        )  # total number of EC2s needed
        if running_count < ec2_needed_count:
            need_count = (
                ec2_needed_count - running_count
            )  # number of more EC2s needed
            if stopped_count > 0:
                print("-> starting a existing EC2 instance")
                if need_count <= stopped_count:
                    stopped_list = stopped_list[:need_count]
                need_count = max(
                    0, need_count - stopped_count
                )  # numebr of EC2s needed more after starting the stopped EC2s
